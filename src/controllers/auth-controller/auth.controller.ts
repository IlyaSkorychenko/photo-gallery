...
import { GetCognitoUser } from 'src/controllers/common/decorators/get-user.decorator';
...
export class AuthController {
  ...
  activateTwoFactor(@GetCognitoUser() user: ICognitoAccessUser) {
    return this.authService.activateTwoFactorByAccessToken(user.accessToken);
  }

  @Post('deactivate-two-factor')
  @UseGuards(AuthGuard('cognito-access'))
  deactivateTwoFactor(@GetCognitoUser() user: ICognitoAccessUser) {
  ...
  @UseGuards(AuthGuard('cognito-access'))
  verifyTwoFactor(
    @GetCognitoUser() { accessToken }: ICognitoAccessUser,
